...
class MarsDayBeforeYesterdayFragment : Fragment(R.layout.fragment_mars) {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        getData(formatter.format(getForthDate()), MainMarsFragment.camera)
    }

    private fun getData(date: String, camera: String) {
        viewModel.getData(date, camera)
            .observe(this@MarsDayBeforeYesterdayFragment, Observer<MarsData> { renderData(it) })
    }
                ...
                is MarsData.Success -> {
                    ...
                    if (image.isNullOrEmpty()) {
                        ...
                        marsImageView.load(image) {
                            lifecycle(this@MarsDayBeforeYesterdayFragment)
