...
class MarsYesterdayFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        getData(formatter.format(getThirdDate()), MainMarsFragment.camera)
    }

    private fun getData(date: String, camera: String) {
        viewModel.getData(date, camera)
            .observe(this@MarsYesterdayFragment, Observer<MarsData> { renderData(it) })
    }
                ...
                is MarsData.Success -> {
                    ...
                    if (image.isNullOrEmpty()) {
                        ...
                        marsImageView.load(image) {
                            lifecycle(this@MarsYesterdayFragment)
